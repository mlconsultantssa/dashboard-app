...
def load_distinct_vehicles():
        ...
        camera_event_value_counts = session.query(CameraEvent.number_plate, func.count('*'))\
            .group_by(CameraEvent.number_plate)\
            .all()
        
        if (camera_event_value_counts):
            camera_event_value_counts.sort(key = lambda tup: tup[1], reverse=True)
